...
- **Kubernetes service-account-token auth with reconnect-on-rotation.** New `KubernetesServiceAccountTokenCredentialsProvider` reads a SAT from a mounted file (default `/var/run/secrets/kubernetes.io/serviceaccount/token`) and presents it as the MQTT password (optional fixed username), and watches the file — periodic polling (5-min default) plus a `FileSystemWatcher` — so when the kubelet rotates the token the client reconnects to present the fresh one. Configure via `MqttClientBuilder.WithKubernetesServiceAccountToken(...)`. Enabled by two supporting additions: `MqttClient.ReconnectAsync(...)` (force a credential-refreshing reconnect) and the optional `IMqttCredentialsChangeNotifier` change-signal interface that the client subscribes to. BCL-only and AOT-clean; the client disposes an `IDisposable`/`IAsyncDisposable` credentials provider on disposal.
