...
{
    /// <summary>
    /// SASL-style method name, e.g. <c>"SCRAM-SHA-256"</c>, <c>"OAUTH-BEARER"</c>.
    /// </summary>
    string Method { get; }
    ...
    /// </summary>
    ValueTask<MqttAuthenticationResult> ContinueAsync(
...
public readonly struct MqttAuthenticationResult
{
    ...
    /// <summary>
    /// Send <paramref name="data"/> in AUTH 0x18 (Continue) and expect another inbound AUTH.
    /// </summary>
    public static MqttAuthenticationResult Continue(ReadOnlyMemory<byte> data)
        => new(
            MqttAuthenticationResultKind.Continue,
            data,
    ...
    /// <summary>
    /// Abort the handshake with the given reason. The client sends DISCONNECT and throws.
    /// </summary>
    public static MqttAuthenticationResult Abort(
        MqttReasonCode reasonCode = MqttReasonCode.NotAuthorized,
